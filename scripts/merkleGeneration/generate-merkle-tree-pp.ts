...
async function main() {
  ...
  const balances = JSON.parse(
    fs.readFileSync(
      `scripts/balanceMapGeneration/${opts.token}/outputBalanceMapsJan24/balanceMap.${network.name}.json`,
      {
        encoding: 'utf8',
      }
    )
  ...
  fs.writeFileSync(
    `scripts/merkleGeneration/outputTrees/${opts.token}/jan24.${network.name}.json`,
